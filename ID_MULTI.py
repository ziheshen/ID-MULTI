...
from modules import CLIPEmbedderWrapper
...
class ID_MULTI(nn.Module):
    def __init__(
        ...
        # Image Encoder
        self.ctx_clip_encoder = CLIPEmbedderWrapper( modifier_token=re.findall(r'<.*?>', args.text_prompt), num_imgs=args.train_batch_size)
        ...
        ### Other Settings ###
        self.loss_w = args.loss
        self.loss_w_1 = args.loss_1
        self.loss_w_2 = args.loss_2
        self.freeze_modules()
        # self.ctx_embeddings_cache = nn.Parameter(
        #     torch.zeros(1, self.num_query_token, 768), requires_grad=False
        # )
        self.ctx_embeddings_cache = {}
        ...
        # inference-related
        # self._CTX_BEGIN_POS = args.ctx_begin_pos
        ...
        self.ctx_clip_encoder.add_token()
    ...
    def load_pretrained_parameters(self):
        ...
        load_state_dict(self.ctx_clip_encoder.transformer, "text_encoder/pytorch_model.bin")
    ...
    def before_training(self, dataset, **kwargs):
        ...
        for subject in dataset.subject:
            input_images[subject] = (
                torch.stack([example["inp_images"][subject] for example in examples])
        ...
        ctx_embeddings={}
        for subject in dataset.subject:
            ctx_embeddings[subject] = self.forward_ctx_embeddings(
                input_image=input_images[subject], text_input=[subject]*input_images[subject].shape[0]
            )
            # take mean of all ctx embeddings
            ctx_embeddings[subject] = ctx_embeddings[subject].mean(dim=0, keepdim=True)
            self.ctx_embeddings_cache[subject] = nn.Parameter(ctx_embeddings[subject], requires_grad=True)
    ...
    def forward(self, samples):
        ...
        ctx_embeddings = {}
        # shpae( bsz, 16, 768 )
        for subject in samples["subject_text"][0]:
            ctx_embeddings[subject] = self.forward_ctx_embeddings(
                input_image=samples["inp_images"][0][subject], text_input=[subject]*bsz
            )
            # print(ctx_embeddings[subject].shape)
        ...
        ### 將每個VQA的答案embedding，並存在字典當中
        vqa_embedder = self.ctx_clip_encoder.transformer.text_model.embeddings.embed
        vqa_embeddings = {}

        for token, ans in samples['vqa_token'][0].items():
            # print(token, ans)
            tokenized_output = self.ctx_clip_encoder.tokenizer(ans, return_tensors='pt')
            vqa_inp_ids = torch.tensor(tokenized_output["input_ids"])
            vqa_embeddings[token] = vqa_embedder( vqa_inp_ids )

        
        encoder_hidden_states, embed_hidden_states = self.ctx_clip_encoder(
            text = samples['caption'],
            input_img = samples['inp_images'],
            ctx_embeddings = ctx_embeddings,
            subjects_position = samples['subjects_position'],
            )
        ...
        id_rel_pred = self.unet(
            noisy_latents.float(), timesteps, encoder_hidden_states[0]
        ).sample
        # id_irrel_pred = self.unet(
        #     noisy_latents.float(), timesteps, img_state + img_state
        # ).sample
        ...
        loss_anchor = {}
        for token, embedding in vqa_embeddings.items():   
            # 找出有效索引，過濾掉找不到 token 的情況
            valid_indices = [
                (i, sentence.split().index(token))
                for i, sentence in enumerate(samples["caption"])
                if token in sentence.split()
            ]

            # 如果有有效索引，繼續選取 hidden states
            if valid_indices:
                batch_indices, seq_indices = zip(*valid_indices)
                batch_indices = torch.tensor(batch_indices, device='cuda')
                seq_indices = torch.tensor(seq_indices, device='cuda')
                
                selected_hidden_states = embed_hidden_states[batch_indices, seq_indices, :]
                
                expand_selected_hidden_states = selected_hidden_states.unsqueeze(1)
                loss_anchor[token] = F.mse_loss( embedding.float(), expand_selected_hidden_states.float() )
        # Compute the average loss over all tokens
        if loss_anchor:
            avg_loss = torch.mean(torch.stack(list(loss_anchor.values())))
        else:
            # Set avg_loss to zero if there are no losses
            avg_loss = torch.tensor(0.0, device='cuda')
        loss = loss + avg_loss
        # for t, l in loss_anchor.items():
        #     loss = loss + l
        ...
        # loss = self.loss_w*loss + self.loss_w_1*loss_aux1 + self.loss_w_2*loss_aux2
    ...
    def forward_ctx_embeddings(self, input_image, text_input, ratio=None):
        ...
        if self._use_embeddings_cache:
            ...
            ctx_embeddings = self.ctx_embeddings_cache[text_input[0]].expand(len(text_input), -1, -1)
        else:
    ...
    def generate(
        ...
        if do_classifier_free_guidance:
            max_length = self.ctx_clip_encoder.transformer.text_model.config.max_position_embeddings

            uncond_input = self.ctx_clip_encoder.tokenizer(
                [neg_prompt],
                padding="max_length",
                max_length=max_length,
                return_tensors="pt",
            )
            uncond_embeddings, _ = self.ctx_clip_encoder.transformer(
            ...
            text_embeddings = torch.cat([uncond_embeddings[0],text_embeddings])
        
        prompt_embeds = text_embeddings
        # prompt_embeds = (text_weight*text_embeddings + image_weight*img_feature) if ref_image is not None else text_weight*text_embeddings
    ...
    def _tokenize_text(self, text_input, with_query=True):
        max_len = self.ctx_clip_encoder.transformer.text_model.config.max_position_embeddings
        if with_query:
            max_len = max_len-self.num_query_token
        self.tokenizer = CLIPTokenizer.from_pretrained(
    ...
    def _forward_prompt_embeddings(self, input_image, src_subjects, prompt, subjects_position):
        ...
        for src_subject in src_subjects:
            query_embeds[src_subject] = self.forward_ctx_embeddings(input_image, src_subject).to(self.device())
    ...
    def _load_checkpoint(self, url_or_filename):
        ...
        if is_url(url_or_filename):
            ...
            if url_or_filename.endswith(".safetensors"):
                checkpoint = load_file(url_or_filename, device="cpu")
            elif url_or_filename.endswith((".pth", ".bin")):
